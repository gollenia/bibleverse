...
class Verse {
    ...
    /**
     * Create a cleaned string with verses to retrieve
     *
     * @param string $verses
     * @return string 
     */
    private static function clean_verse(string $verses) {
    ...
    static function where(Book $book, int $chapter, string $verse = "") {
        $verses = empty($verse) ? "" : " AND verse IN (" . self::clean_verse($verse) .")" ;
